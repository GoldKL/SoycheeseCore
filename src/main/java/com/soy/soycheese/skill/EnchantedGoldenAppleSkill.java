...
public class EnchantedGoldenAppleSkill extends BaseSkill {
    ...
    public List<Component> getDescription(Player player) {
        ...
        {
            list.add(Component.translatable(getOrCreateNameid() + ".lock"));
        }
        else
        {
            list.add(Component.translatable(getOrCreateNameid() + ".unlock_0"));
            list.add(Component.translatable(getOrCreateNameid() + ".unlock_1"));
        }
        return list;
    }
